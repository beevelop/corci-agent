...
function GenericBuild(build, agent, command) {
    ...
    if (agent.platform) {
        var platform = new agent.platform(build, agent);
        this.setHook('filesDone', platform.filesDone.bind(platform));
        this.setHook('buildDone', platform.buildDone.bind(platform));
        this.setHook('preCordovaBuild', platform.preCordovaBuild.bind(platform));
        platform.init();
    }
}
...
GenericBuild.prototype.s7BuildCordova = function (err, args) {
    ...
    if (_build.conf.platform === 'android') {
        if (os.platform() === 'linux') {
            ...
            var cordovaBuildFile = path.resolve(this.locationPath, 'platforms/android/cordova/build');

            //@TODO: async?
            if (fs.existsSync(cordovaBuildFile)) {
                fs.chmodSync(cordovaBuildFile, '755', function (err) {
                    if (err) {
                        var _msg = 'Permission for {2} could not be set!';
                        _agent.log(_build, Msg.info, _msg, cordovaBuildFile);
                    }
                });
            }
        } else {
