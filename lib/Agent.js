...
Agent.prototype.connect = function (url) {
    ...
    socket.on('transfer', this.onTransfer.bind(this));
...
Agent.prototype.onTransfer = function (stream, meta, BID) {
    var localpath = path.resolve(this.getWorkfolder(), BID, meta.basename);

    var _this = this;
    FileStream.save(stream, localpath)
        .then(function () {
            _this.log.agent('Successfully received file for Build #%s!', BID);
            _this.addFile(BID, localpath);
        })
        .catch(function (err) {
            _this.log.warn('Could not save file for Build #%s!', BID, err);
...
Agent.prototype.onDisconnect = function () {
    Logger.verbose('AgentWorker diconnected! Affected platform: %s', this.getPlatform());
};
